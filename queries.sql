...
select se.IdSeller, count(*) as CountOrders, sum(ps.Price) as TotalAmount
from ProductSellers as ps inner join Sellers as se on (ps.IdSeller = se.IdSeller)
group by se.IdSeller order by CountOrders ASC

select s.Usename, count(od.Id) as CountOrders, sum(od.TotalPrice) as TotalAmount
from Sellers as s 
